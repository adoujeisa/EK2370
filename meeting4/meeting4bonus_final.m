...
% Read the audiofile
[y,Fs] = audioread('fmcw_single1.wav');%single_target_range.wav'); 
...
%% compute use other method
 rangeExtracted2 = 15*(fridge_up*c*Tp)/bandwidth;
 velExtracted2 = zeros(numRidges,length(rangeExtracted2));
 velExtracted2(:,5:end) = (rangeExtracted2(5:end,:)' - rangeExtracted2(1:end-4,:)')/(5*Tp);
...
% Plot the Range vs. Time use other method
figure;
plot(time, rangeExtracted2);
title('Range vs Time(new method for bonus)');
xlabel('Time [s]');
ylabel('Range [m]');
grid on;

figure;
velocityPlot = plot(time,velExtracted2);
title("Velocity vs Time plot(new method for bonus)"); 
xlabel('Time [s]'); 
ylabel('velocity [m/s]');
